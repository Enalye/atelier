...
struct Vec2(T) {
    ...
    Vec2!T getNearestPoint(const Vec2!T a, const Vec2!T b) const {
        Vec2!T ab = b - a;
        Vec2!T ap = this - a;

        static if (__traits(isFloating, T)) {
            T t = ap.dot(ab) / ab.lengthSquared();
            ...
            double t = ap.dot(ab) / cast(double) ab.lengthSquared();
            ...
            return a + cast(Vec2!T)(t * cast(Vec2f) b).round();
        }
    }

    static if (__traits(isFloating, T)) {
        /// Renvoie le point sécant entre deux segments (a1;b1) et (a2;b2)
        static auto getIntersection(Vec2!T a1, Vec2!T b1, Vec2!T a2, Vec2!T b2) {
            struct Result {
...
(float a1.x, float a1.y, float b1.x, float b1.y, 
     ...
     */
            ...
            T s = (-s1.y * (a1.x - a2.x) + s1.x * (a1.y - a2.y)) * d;
            T t = (s2.x * (a1.y - a2.y) - s2.y * (a1.x - a2.x)) * d;

            result.hasIntersection = s >= 0 && s <= 1 && t >= 0 && t <= 1;
            result.point = a1 + t * s1;
