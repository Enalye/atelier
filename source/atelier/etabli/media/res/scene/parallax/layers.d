module atelier.etabli.media.res.scene.parallax.layers;
...
import atelier.etabli.media.res.scene.parallax.add;
import atelier.etabli.media.res.scene.parallax.duplicate;
import atelier.etabli.media.res.scene.parallax.edit;
...
package(atelier.etabli.media.res.scene) final class ParallaxList : UIElement {
    private {
        ...
        SceneDefinition.ParallaxLayer _currentLayer;
    ...
    this(SceneDefinition definition) {
        ...
        {
            _parallaxList = new VList;
            _parallaxList.setSize(Vec2f(300f, 250f));
    ...
    void startTool(Vec2f mousePos) {
        ...
        enum tileSize = 16;
        Vec2i dimensions = Vec2i(_currentLayer.getWidth(), _currentLayer.getHeight());
        _mapSize = (cast(Vec2f) dimensions * tileSize) * _zoom;
        Vec2f offset = _centerPosition + _mapPosition / _currentLayer.distance;
    ...
    private void updateSelectionPreview() {
        if (!_currentLayer || !_currentLayer.tileset)
            return;

        _previewSelectionTM = new Tilemap(_currentLayer.tileset,
    ...
    private void _rebuildList() {
        _parallaxList.clearList();
        foreach (layer; _definition.getParallaxLayers()) {
            LayerElement elt = new LayerElement(this, layer);
            _parallaxList.addList(elt);
        }
    }

    private void _select(SceneDefinition.ParallaxLayer layer) {
        closeToolbox();

        _currentLayer = layer;
        size_t i;
        foreach (item; cast(LayerElement[]) _parallaxList.getList()) {
            bool isSelected = item._layer == layer;
            if (isSelected) {
                _parallaxList.moveToElement(i);
            }
    ...
    private void _onAddItem() {
        auto modal = new AddParallaxElement;
        modal.addEventListener("apply", {
            SceneDefinition.ParallaxLayer layer = _definition.createParallaxLayer();
    ...
    private void _onDuplicateItem() {
        ...
        auto modal = new DuplicateParallaxElement(_currentLayer.name);
        modal.addEventListener("apply", {
            SceneDefinition.ParallaxLayer layer = _definition.duplicateParallaxLayer(_currentLayer);
            layer.name = modal.getName();
            _rebuildList();
            _select(layer);
        });
    ...
    private void _onEditItem(SceneDefinition.ParallaxLayer layer) {
        auto modal = new EditParallaxElement(layer);
        modal.addEventListener("apply", {
            layer.name = modal.getName();
            layer.tilesetRID = modal.getTileset();
            layer.distance = modal.getDistance();
            layer.setSize(modal.getWidth(), modal.getHeight());
            _rebuildList();
            _select(layer);
        });
        Atelier.ui.pushModalUI(modal);
    ...
    private void _onRemoveItem() {
        ...
        auto modal = new RemoveParallaxElement(_currentLayer);
        modal.addEventListener("apply", {
    ...
    private void moveUp(SceneDefinition.ParallaxLayer layer) {
        layer.moveUp();
        _rebuildList();
        _select(layer);
    }

    private void moveDown(SceneDefinition.ParallaxLayer layer) {
        layer.moveDown();
        _rebuildList();
        _select(layer);
    }
...
package final class LayerElement : UIElement {
    private {
        ...
        Rectangle _rect;
        Label _label;
        VisibilityToggle _visibleToggle;
        bool _isSelected;
        HBox _hbox;
    ...
    this(ParallaxList rlist, SceneDefinition.ParallaxLayer layer) {
        _list = rlist;
        _layer = layer;
        ...
        _visibleToggle = new VisibilityToggle(_layer.isVisible);
        _visibleToggle.setAlign(UIAlignX.left, UIAlignY.center);
        _visibleToggle.setPosition(Vec2f(4f, 0f));
        _visibleToggle.addEventListener("value", {
            _layer.isVisible = _visibleToggle.value();
        });
        addUI(_visibleToggle);

        _label = new Label(format("%s (%f)", _layer.name, _layer.distance), Atelier.theme.font);
