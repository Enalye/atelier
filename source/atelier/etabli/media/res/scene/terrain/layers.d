module atelier.etabli.media.res.scene.terrain.layers;
...
import atelier.etabli.media.res.scene.terrain.add;
import atelier.etabli.media.res.scene.terrain.duplicate;
import atelier.etabli.media.res.scene.terrain.edit;
...
package(atelier.etabli.media.res.scene) final class TerrainList : UIElement {
    private {
        ...
        SceneDefinition.TerrainLayer _currentLayer;
    ...
    private void _select(SceneDefinition.TerrainLayer layer) {
        closeToolbox();
    ...
    private void _onAddItem() {
        auto modal = new AddTerrainElement;
        modal.addEventListener("apply", {
            SceneDefinition.TerrainLayer layer = _definition.createTerrainLayer();
    ...
    private void _onDuplicateItem() {
        ...
        auto modal = new DuplicateTerrainElement(_currentLayer.name);
        modal.addEventListener("apply", {
            SceneDefinition.TerrainLayer layer = _definition.duplicateTerrainLayer(_currentLayer);
            layer.name = modal.getName();
            _rebuildList();
            _select(layer);
        });
    ...
    private void _onEditItem(SceneDefinition.TerrainLayer layer) {
        auto modal = new EditTerrainElement(layer);
        modal.addEventListener("apply", {
            layer.name = modal.getName();
            layer.terrainRID = modal.getTerrain();
    ...
    private void _onRemoveItem() {
        ...
        auto modal = new RemoveTerrainElement(_currentLayer);
        modal.addEventListener("apply", {
    ...
    private void moveUp(SceneDefinition.TerrainLayer layer) {
        layer.moveUp();
        _rebuildList();
        _select(layer);
    }

    private void moveDown(SceneDefinition.TerrainLayer layer) {
        layer.moveDown();
        _rebuildList();
        _select(layer);
    }
...
package final class LayerElement : UIElement {
    ...
    this(TerrainList rlist, SceneDefinition.TerrainLayer layer) {
        _list = rlist;
        _layer = layer;
