module atelier.etabli.media.res.tilemap.editor;
...
import atelier.etabli.media.res.tilemap.toolbox;
import atelier.etabli.media.res.tilemap.parameter;
import atelier.etabli.media.res.tilemap.selection;
...
final class TilemapResourceEditor : ResourceBaseEditor {
    ...
    this(ResourceEditor editor, string path_, Farfadet ffd, Vec2f size) {
        ...
        _toolbox.setTileset(_tilesetRID);
    ...
    void setTilesetRID(string rid) {
        ...
        if (Atelier.etabli.hasResource("tileset", rid)) {
            auto tilesetRes = Atelier.etabli.getResource("tileset", rid);
            string textureRID = tilesetRes.farfadet.getNode("texture").get!string(0);
            if (Atelier.etabli.hasResource("texture", textureRID)) {
                auto textureRes = Atelier.etabli.getResource("texture", textureRID);
