module atelier.etabli.media.texteditor;
...
class TextEditor : ContentEditor {
    ...
    void onCtrlLeftClick() {
    }

    void onCtrlRightClick() {
    }

    private void _onMouseDown() {
        InputEvent.MouseButton event = getManager().input().asMouseButton();
        switch (event.button) with (InputEvent.MouseButton.Button) {
        case left:
            uint line = _getMouseLine();
            uint column = _getMouseColumn(line);
    ...
    void setLinter(UIElement element) {
        if (_linter == element)
            return;
    ...
    private void _relocateLinter() {
        ...
        _linter.setPosition(Vec2f(64f + columnOffset * charSize,
                (lineOffset + 1) * _font.lineSkip()));
    }

    final void updateLines(bool useView = false) {
        uint textWindowHeight = cast(uint)(getHeight() / _font.lineSkip());
