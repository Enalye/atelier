module atelier.etabli.ui.fileexplorer;
...
final class FileExplorer : Surface {
    ...
    this() {
        ...
        addEventListener("size", {
            if (_list) {
                _list.setHeight(max(0f, getHeight() - 104f));
            }
        });
    }
    ...
    void reload() {
        ...
        _list = new VList;
        _list.setSize(Vec2f(getWidth(), max(0f, getHeight() - 104f)));
        vbox.addUI(_list);
    ...
    void rebuildList() {
        ...
        _currentMedia = _mediaSelect.value;
        folderPath = buildNormalizedPath(Atelier.etabli.getMediaDir(), _currentMedia);

        if (search.length) {
            if (exists(folderPath)) {
                foreach (entry; dirEntries(folderPath, SpanMode.depth)) {
                    if (entry.isDir() || entry.toLower.indexOf(search) == -1)
                        continue;
...
private final class FileItem : Item {
    this(FileExplorer rlist, string path_, uint depth_) {
        ...
        case ".ffd":
            ...
            break;
        case ".seq":
            icon = new Icon("editor:file-sequencer");
            break;
        default:
