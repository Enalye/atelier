 ...
 * Authors: Enalye
 */
module atelier.ui.input.textfield;
...
final class TextField : UIElement {
    ...
    private void _onFocus() {
        ...
        _outline.color = Atelier.theme.accent;
    }
    ...
    private void _onKeyButton() {
        ...
        if (!manager.input.isPressed())
            return;

        InputEvent.KeyButton ev = manager.input.asKeyButton();

        switch (ev.button) with (InputEvent.KeyButton.Button) {
        case right:
            if (_selectionIndex != _caretIndex) {
                if (!Atelier.input.isPressed(InputEvent.KeyButton.Button.leftShift) &&
        ...
        case v:
            if (Atelier.input.isPressed(InputEvent.KeyButton.Button.leftControl) ||
                ...
                if (Atelier.input.hasClipboard()) {
                    _insertText(to!dstring(Atelier.input.getClipboard()));
                }
            }
            break;
        case c:
            if (Atelier.input.isPressed(InputEvent.KeyButton.Button.leftControl) ||
                Atelier.input.isPressed(InputEvent.KeyButton.Button.rightControl)) {
                Atelier.input.setClipboard(_getSelection());
            }
            break;
        case x:
            if (Atelier.input.isPressed(InputEvent.KeyButton.Button.leftControl) ||
